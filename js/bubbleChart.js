...
BubbleChart.prototype.initVis = function () {
  ...
  vis.mechanics = {
    ...
    4: "Modular Board",
    5: "Role Playing",
    6: "Simultaneous Action Selection",
    7: "Storytelling",
    8: "Variable Player Powers",
    9: "Action Point Allowance System",
    ...
    15: "Take That",
    16: "Tile Placement",
    17: "Area Control / Area Influence",
    18: "Dice Rolling",
    19: "Area Movement",
    20: "Partnerships",
    21: "Route/Network Building",
    22: "Deck / Pool Building",
    23: "Variable Phase Order",
    24: "Area-Impulse",
    25: "Voting",
    26: "Action / Movement Programming",
    27: "Worker Placement",
    28: "Area Enclosure",
    29: "Memory",
    30: "Pattern Building",
    31: "Press Your Luck",
    32: "Pick-up and Deliver",
    33: "Player Elimination",
    34: "Secret Unit Deployment",
  ...
  vis.categories = {
    ...
    13: "Territory Building",
    14: "Modern Warfare",
    15: "Political",
    16: "Wargame",
    17: "Movies / TV / Radio theme",
    18: "American West",
    19: "Animals",
    20: "Negotiation",
    21: "Novel-based",
    ...
    64: "Maze",
    65: "Pirates",
    66: "Expansion for Base-game",
    67: "Sports",
    68: "Educational",
    69: "American Indian Wars",
...
BubbleChart.prototype.wrangleData = function () {
  ...
  vis.mechs_cat = d3.select("#mechs_cat").property("value");
...
BubbleChart.prototype.updateVis = function () {
  ...
  (vis.min = d3.min(vis.nested_data, function (d) {
    return d.value;
  })),
    (vis.max = d3.max(vis.nested_data, function (d) {
      return d.value;
    })),
    (vis.radiusScale = d3
      .scaleSqrt()
  ...
  vis.elem = vis.svg.selectAll("g").data(vis.nested_data);

  vis.elem.exit().transition().duration(500).style("opacity", 1e-6).remove();

  //new
  vis.newsGroup = vis.elem.enter();
  vis.newsGroup = vis.newsGroup
    .append("g")
    .attr("class", "node")
  ...
  vis.elem
    ...
    .on("mouseover", (d) => {
      if (vis.mechs_cat == "top_mechanic_idx") {
        vis.div.transition().duration(200).style("opacity", 0.9);
        vis.div
  ...
  vis.elem
    ...
    .style("font-size", function (d) {
      if (vis.mechs_cat == "top_mechanic_idx") {
        var len = vis.mechanics[d.key].substring(
          0,
          vis.radiusScale(d.value) / 5
        ).length;
        var size = d.r / 3;
        size *= 10 / len;
        size += 1;
        ...
        var len = vis.categories[d.key].substring(
          0,
          vis.radiusScale(d.value) / 5
        ).length;
        var size = d.r / 3;
    ...
    .text(function (d) {
      if (vis.mechs_cat == "top_mechanic_idx") {
        var text = vis.mechanics[d.key].substring(
          0,
          vis.radiusScale(d.value) / 5
        );
        ...
        var text = vis.categories[d.key].substring(
          0,
          vis.radiusScale(d.value) / 5
        );
  ...
  vis.simulation.nodes(vis.nested_data).on("tick", ticked);

  vis.simulation.alpha(0.8).restart();
  vis.highlightCircles();

  function ticked() {
    vis.elem.attr("transform", function (d) {
...
BubbleChart.prototype.highlightCircles = function () {
  ...
  d3.selectAll("g.node").each(function (d) {
    d3.select(this).selectAll("#data").style("stroke-width", "0px");
      ...
      d3.select(this)
        ...
        .each(function (d) {
          ...
          if (selectedClickedMechs.has(String(current.attr("class")))) {
            current.style("stroke", "white").style("stroke-width", "8px");
          }
        });
